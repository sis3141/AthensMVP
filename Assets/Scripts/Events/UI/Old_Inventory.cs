...
public class Old_Inventory : MonoBehaviour
{
    ...
    public float slotSize = 100;              // 슬롯의 사이즈.
    public float slotGap = 2;               // 슬롯간 간격.
    public float slotCountX = 4;            // 슬롯의 가로 개수.
    public float slotCountY= 4;            // 슬롯의 세로 개수.
    ...
    void Awake()
    {
        ...
        for (int y = 0; y < slotCountY; y++)
        {
            for (int x = 0; x < slotCountX; x++)
            {
                ...
                // 슬롯이 생성될 위치 설정하기.
                slotRect.localPosition = new Vector3((slotSize * x) + (slotGap * (x + 1)),-((slotSize * y) + (slotGap * (y + 1))),0);
